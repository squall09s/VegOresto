...
extension AAPopUp {
    ...
    func keyboardDidChange(notification: NSNotification) {
        ...
        let keyboardHeight: CGFloat = (info[UIKeyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size.height ?? 0.0
        let duration: Double = (info[UIKeyboardAnimationDurationUserInfoKey] as? Double) ?? 0.0
